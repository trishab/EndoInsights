...
def main():
    ...
    year_range = range(
        min(
            genetic_endo_counts.index.min(),
            all_endo_counts.index.min(),
            genetic_ipf_counts.index.min(),
            all_ipf_counts.index.min(),
        ),
        max(
            genetic_endo_counts.index.max(),
            all_endo_counts.index.max(),
            genetic_ipf_counts.index.max(),
            all_ipf_counts.index.max(),
        )
        + 1,
    )
    ...
    plt.plot(
        genetic_endo_counts.index,
        genetic_endo_counts.values,
        label="Genetic Mechanisms of Endometriosis",
        color="blue",
    )
    plt.plot(
        all_endo_counts.index,
        all_endo_counts.values,
        label="All Endometriosis Studies",
        linestyle="--",
        color="lightblue",
    )
    plt.plot(
        genetic_ipf_counts.index,
        genetic_ipf_counts.values,
        label="Genetic Mechanisms of IPF",
        color="green",
    )
    plt.plot(
        all_ipf_counts.index,
        all_ipf_counts.values,
        label="All IPF Studies",
        linestyle="--",
        color="lightgreen",
    )
